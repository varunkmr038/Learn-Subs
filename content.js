...
const captionObserver = new MutationObserver(() => {
  ...
  let captionWindow = document.querySelector(
    'div[id*="caption-window"] > div[class*="caption-window ytp-caption-window-bottom"]'
  );
  // console.log(++count)
  ...
  for (const span of spanList) {
    span.onmouseenter = (event) => {
      mouseEnterEvent(span.textContent, event);
    };
    span.onmouseleave = (event) => {
      mouseLeaveEvent(event.currentTarget);
    };
    span.onclick = (event) => {
      mouseClickEvent(span.textContent, event);
    };
  }
...
function mouseEnterEvent(word, event) {
  ...
  element.style.cssText =
    "background-color: rgba(91, 8, 199, 0.5) !important;  border-radius: 50px !important; ";
}
...
function mouseClickEvent(word, event) {
  ...
  box.className = "dbox";
  box.style.cssText =
    "color: black; border: 2px solid rgba(255, 0,0, 1); width:30%; position: absolute; padding: 10px; margin: 20px; border-radius: 20px; z-index: 9999 !important; background-color: rgba(255, 255, 255, 1) !important;";

  let header = document.createElement("div");
  header.style.cssText = "display: flex; justify-content:center";
  dcross = document.createElement("h2");
  dcross.className = "dcross";
  dcross.innerHTML = "Learn Subs 🧾";
